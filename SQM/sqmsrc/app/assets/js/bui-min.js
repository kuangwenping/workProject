...
/**
 * @fileOverview 隐藏，显示 挂件
 * @ignore
 */
define('czy/shrink', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        Shrink = BUI.namespace('Shrink');

    Shrink.display = function (items) {
        this.btn = items.btn;
        this.moverEl = items.moveEl;
        this.movesway = items.way;

        this.moveLength = 0;
        this.bound = items.bound;
        this.init();
    };

    Shrink.display.prototype = {
        init: function () {
            this.moveLengthFun(this.bound);
            this.move(this.moveLength);
            this.event();
        },

        moveLengthFun: function (dist) {
            var way = this.movesway === 1 ? $(this.moverEl).width() : $(this.moverEl).height();
            this.moveLength += way * dist;
        },

        move: function (length) {
            this.movesway === 1 ? $(this.moverEl).css({
                right: length + 'px',
                transition: 'all 1s'
            }) : $(this.moverEl).css({
                marginTop: length + 'px',
        ...
        event: function () {
            ...
            $(_this.btn).on('click', function (e) {
                e.preventDefault();
                console.log($(this).siblings(_this.moveEl));
                _this.bound = _this.bound === -1 ? 1 : -1;
                _this.moveLengthFun(_this.bound);
                _this.move(_this.moveLength);
            })
        }
    }
    return Shrink;
...
/**
 * @fileOverview 隐藏，显示 块
 * @ignore
 */
define('czy/slideThunk', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        slideThunk = BUI.namespace('slideThunk');

    slideThunk.display = function (items) {
        this.btn = items.btn;
        this.moverEl = items.moveEl;
        this.cb = items.cb;
        this.flg = null;//开关
    ...
    slideThunk.display.prototype = {
        init: function () {
            var _this = this;
            $(_this.btn).on('click', function (e) {
                e.preventDefault();
                var flg = parseInt($(this).attr('data-flg'));
                var length = 0;
                var $moverObj = $(this).siblings(_this.moveEl);

                if (flg) {
                    $(this).attr('data-flg', 0);
                    length = _this.moveLength(flg, $moverObj);
                } else {
                    $(this).attr('data-flg', 1);
                    length = _this.moveLength(flg, $moverObj);
                }

                _this.move(length, $moverObj);
                _this.cb({flg: flg, $currBtnEl: $(this), $currMoveEl: $moverObj});
            })
        },

        moveLength: function (flg, $moverObj) {
            return flg ? 0 : $moverObj.outerHeight(true);
        },

        move: function (length, $moverObj) {
            $moverObj.css({
                marginTop: -length + 'px',
                transition: 'all 1s'
            });
...
/**
 * @fileOverview checkbox  全选，反选
 * @ignore
 */
define('czy/check', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        check = BUI.namespace('check');

    check.Select = function (deploy) {
        this.deploy = deploy || null;
        this.deploy.selecteds = [];
        this.init();
        if (deploy.callBack && typeof deploy.callBack === 'function') {
            this.callBack = deploy.callBack;
        } else {
            this.callBack = null;
        }
    };

    check.Select.prototype = {
        init: function () {
            this.deploy.autoRender && this.render(); //自动渲染
        },
        //绘制
        render: function () {
            var items = this.deploy.items, //取出配置item
                parEl = this.deploy.rendEl, //获取父元素
                bart = this.deploy.tbar || [],    //获取bar配置
                name = this.deploy.name || '',
                elName = this.deploy.elName || 'li',
                parElName = elName === 'li' ? 'ul' : 'div',
                elCls = this.deploy.elCls || '';  //获取包裹层类名

            var childEl = '';

            //调用单元绘制
            var tpl = this.unitRender({elName: elName, elData: items, classstr: 'select-item', name: name});

            //注入单元包裹层
            var ulEl = '<' + parElName + ' class="' + elCls + '">' + tpl + '</' + parElName + '>';

            //是否绘制bart元素
            if (bart.length) {
                var barChild = this.unitRender({
                    elName: 'button',
                    elData: bart,
                    classstr: 'czy-btn  czy-btn-x czy-btn-empty'
                });
                var barEl = '<div class="selece-button-group">' + barChild + '</div>';
                childEl = '<div>' + barEl + ulEl + '</div>';
            } else {
                childEl = ulEl;
            }

            //注入父级元素
            $(parEl).append(childEl);
            //调用bar方法
            this.barevent();
            //配置拖拽
            this.deployConfig('itemEl', $(parEl).find('.select-item'));
            //初始化事件
            this.event();
            //绘制样式
            this.styleRender();
        ...
        //单元绘制方法
        unitRender: function (parame) {
            var unitEl = '';
            if (parame.elData.length) {
                $(parame.elData).each(function (i, item) {
                    var elClass = item.elCls || '',
                        itemId = item.id || '',
                        itemText = item.text || '',
                        childTpl = item.childTpl || '',
                        beforeTpl = item.beforeTpl || '',
                        value = item.value || '',
                        checked = item.checked ? 1 : 0,
                        disabledCls = item.disabled ? 'czy-disabled' : '';

                    unitEl += '<' + parame.elName + ' data-name="' + parame.name + '" data-check="' + checked + '"  class="' + parame.classstr + " " + elClass + " " + disabledCls + '" id="' + itemId + '" data-value="' + value + '">' + beforeTpl + itemText + childTpl + '</' + parame.elName + '>';
                });
            }
            return unitEl
        },
        ...
        barevent: function () {
            var _this = this;
            var tbar = this.deploy.tbar;
            $(tbar).each(function (v, item) {
                if (item.listeners && item.id || item.elCls) {
                    for (var eventStr in item.listeners) {
                        var eventEl = item.id && "#" + item.id || "." + item.elCls;
                        _this.barEventExecute(eventEl, eventStr, item.listeners[eventStr]);
                    }
                }
            });
        },
        ...
        //bar事件函数调用
        barEventExecute: function (el, eventName, cbFun) {
        ...
        //参数配置
        deployConfig: function (key, val) {
            this.deploy[key] = val;
        },

        event: function () {
            var _this = this;
            this.deploy.itemEl.on('click', function (e) {
                e.preventDefault();
                if ($(this).hasClass('czy-disabled')) {
                    return
                }
                var val = $(this).attr('data-value');
                var idx = $(this).index();
                if (_this.deploy.pattern === 'check') {
                    _this.checkSelect(this);
                    _this.callBack && _this.callBack($(this));
                } else if (_this.deploy.pattern === 'radio') {
                    _this.radioSelect(this);
                    _this.callBack && _this.callBack($(this));
                } else {
                    return
                }
            ...
            this.deploy.itemEl.on('mouseenter', function (e) {
                _this.tooltipRender($(this));
            })

            this.deploy.itemEl.on('mouseleave', function (e) {
                $('.check-tooltip').length && _this.tooltipNone($('.check-tooltip'));
                return
            })
        },
        //tooltip渲染
        tooltipRender: function ($currEl) {
            var _this = this;
            var index = $currEl.index();
            var tooltipText = this.deploy.items[index].tooltip || null;
            if (!tooltipText) {
                return
            }
            ;
            var currOffset = $currEl.offset();

            if (!$('.check-tooltip').length) {
                var tooltipEl = '<div class="check-tooltip czy-none">' + tooltipText + '</div>';
                $('body').append(tooltipEl);
            } else {
                $('.check-tooltip').text(tooltipText);
            }

            $('.check-tooltip').css({
                display: 'inline-block',
                top: currOffset.top + 35 + 'px',
                left: currOffset.left + 'px',
            });

            $('.check-tooltip').on('mouseleave', function () {
                _this.tooltipNone($(this));
            })
        },

        tooltipNone: function ($currEl) {
            $currEl.css({
                display: 'none'
            })
        },
        ...
        //单选
        radioSelect: function (currEl) {
            $(currEl).attr('data-check', 1).addClass('selected').siblings('.selected').attr('data-check', 0).removeClass('selected');
        },

        //多选
        checkSelect: function (currEl) {
            if (parseInt($(currEl).attr('data-check')) === 1) {
                $(currEl).attr('data-check', 0);
            } else {
                $(currEl).attr('data-check', 1);
            }
            this.styleRender();
        },
        ...
        allCheck: function () {
            var _this = this;
            this.deploy.itemEl.each(function (i, item) {
        ...
        reverseCheck: function () {
            var _this = this;
            this.deploy.itemEl.each(function (i, item) {
                if (parseInt($(item).attr('data-check')) === 1) {
 ...
 * @ignore
 */
define('czy/menuSlide', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        menuSlide = BUI.namespace('cmenuSlide');

    menuSlide.Slide = function (deploy) {
        this.deploy = deploy || null;
        this.deploy.els = null;
        this.deploy.moveEl = null;
        this.deploy.flxEl = null;
        this.deploy.slideBtns = null;
        this.deploy.moveLength = 0;

        this.deploy.leftLimitation = false; //左边极点判断
        this.deploy.rightLimitation = false; //右边极点判断

        this.deploy.differMove = 0; //能够移动的最大距离

        this.init();
    };

    menuSlide.Slide.prototype = {
        init: function () {
            this.deploy.autoRender && this.render();
        },
        //绘制
        render: function () {
            var items = this.deploy.items, //取出配置item
            ...
            //调用单元绘制
            var tpl = this.unitRender({elName: elName, elData: items});

            //注入单元包裹层
            var ulEl = '<' + parElName + ' class="menu-tab-list' + elCls + '">' + tpl + '</' + parElName + '>';

            //tabul包裹限制级
            var childEl = '<div class="menu-tab">' + ulEl + '</div>';

            //箭头操作滑动
            var slideBtnEl = '<div class="slide-btn-group float-r">' +
                '<a class="slide-btn-left"> <i></i> </a>' +
                '<a class="slide-btn-right"> <i></i> </a>' +
                '</div>';

            //注入父级元素
            $(parEl).append(childEl);
            $(parEl).append(slideBtnEl);

            //获取相关元素
            this.deploy.flxEl = $(parEl).find('.menu-tab');
            this.deploy.moveEl = this.deploy.flxEl.find('.menu-tab-list');
            this.deploy.els = this.deploy.moveEl.find('.tab-item');
            this.deploy.slideBtns = $(parEl).find('.slide-btn-group');

            if (this.deploy.moveLength === 0) {
                this.deploy.leftLimitation = true;
            }
            this.slideBtnsStyleRender();
            this.event();
            //计算可移动最大值
            this.deploy.differMove = this.sectionMove();
        ...
        unitRender: function (parame) {
            ...
            if (parame.elData.length) {
                $(parame.elData).each(function (i, item) {
                    ...
                    unitEl += '<' + parame.elName + ' class="tab-item ' + elClass + ' ' + active + '" id="' + itemId + '"> <span>' + itemText + '</span>' + childTpl + '</' + parame.elName + '>';
                });
            }
            return unitEl
        },
        ...
        defaultTab: function () {
            ...
            var defaultItemIndex = null;
            $(this.deploy.items).each(function (i, item) {
                if (item.active) {
                    defaultItemIndex = i;
                }
            });
            $($contents[defaultItemIndex]).show();
        },

        //箭头样式初始化
        slideBtnsStyleRender: function () {
            if (this.deploy.moveEl.width() < this.deploy.flxEl.width(true)) {
                this.deploy.slideBtns.children().addClass('disabled');
            } else {

                if (this.deploy.leftLimitation) {
                    this.deploy.slideBtns.children('.slide-btn-left').addClass('disabled');
                } else {
                    this.deploy.slideBtns.children('.slide-btn-left').removeClass('disabled');
                }

                if (this.deploy.rightLimitation) {
                    this.deploy.slideBtns.children('.slide-btn-right').addClass('disabled');
                } else {
                    this.deploy.slideBtns.children('.slide-btn-right').removeClass('disabled');
                }
            }
        },
        ...
        event: function () {
            var _this = this;
            this.deploy.els.on('click', function () {
                _this.deploy.tabChangerCb && typeof _this.deploy.tabChangerCb === 'function' && _this.deploy.tabChangerCb($(this))
                _this.muenCut($(this));
                _this.slide($(this));
            });

            this.deploy.slideBtns.children('.slide-btn-left').on('click', function () {
                if ($(this).hasClass('disabled')) {
                    return
                }
                _this.deploy.rightLimitation = false;
                _this.slide($(_this.deploy.els[1]), 1);
            });

            this.deploy.slideBtns.children('.slide-btn-right').on('click', function () {
                if ($(this).hasClass('disabled')) {
                    return
                }
                _this.deploy.leftLimitation = false;
                _this.slide($(_this.deploy.els[1]), -1);
            });
        },

        //切换
        muenCut: function ($currTab) {
            var _this = this;
            this.clearRender('active');
            $currTab.addClass('active');
        },

        slide: function (currEl, price) {
            if (!price) {
                //点击菜单移动
                this.deploy.moveLength += this.countMove(currEl, null);

            } else {
                //点击箭头移动
                this.deploy.moveLength += (currEl.outerWidth(true) * price);
                var differ = this.deploy.differMove;
                var movelength = Math.abs(this.deploy.moveLength);

                if (movelength > differ) {
                    this.deploy.rightLimitation = true;
                    this.deploy.moveLength = -differ;
                }
            ...
            this.moverTo(this.deploy.moveLength);
            this.slideBtnsStyleRender();
        },

        //获取移动区间
        sectionMove: function () {
            var flxW = this.deploy.flxEl.width();
            var moveW = this.deploy.moveEl.width();
            var differ = flxW - moveW;
            if (differ > 0) {
                return
            }
        ...
        //点击菜单计算移动距离
        countMove: function (currEl) {
            var flxEl = this.deploy.flxEl;
            var els = this.deploy.els;
            var index = currEl.index();

            //计算当前元素距离固定外层左侧的距离
            var grp = currEl.offset().left - flxEl.offset().left;

            //点击菜单移动  间距为正数时，元素贴近固定外层的右侧，否者靠近元素的左侧；
            if (grp > 0) {
                var sumLength = grp + $(currEl).outerWidth(true),
                    wrapLength = flxEl.width();

                //总距离大于固定宽度
                if (sumLength > wrapLength) {
                    this.deploy.leftLimitation = false;
                    if (index === els.length - 1) {
                        this.deploy.rightLimitation = true;
                        return (wrapLength - sumLength);
                    }
                    return (wrapLength - 20 - sumLength);
                } else {
                    return 0;
                }
            } else if (grp < 0) {
                this.deploy.rightLimitation = false;
                if (index === 0) {
                    this.deploy.leftLimitation = true;
                    return Math.abs(grp);
                }
                return Math.abs(grp) + 20;
            } else {
                return 0;
            }
        },

        moverTo: function (moverLength) {
            if (this.deploy.moveEl.width() < this.deploy.flxEl.width()) {
                return
            }
            this.deploy.moveEl.css({
                marginLeft: moverLength + 'px',
                transition: 'all .5s'
            })
        },

        clearRender: function (cname) {
            this.deploy.els.each(function (i, item) {
                $(item).hasClass(cname) && $(item).removeClass(cname);
            });
        },
    }
 ...
 * @ignore
 */
define('czy/searchSelect', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        czySelect = BUI.namespace('czySelect');

    czySelect.SearchSelect = function (deploy) {
        if (!deploy) {
            return
        }
        this.deploy = deploy || null;
        this.deploy.selectedEls = null;
        this.deploy.selectEls = null;
        this.deploy.selectItemEls = null;
        this.deploy.inputEls = null;

        this.init();
    };

    czySelect.SearchSelect.prototype = {
        init: function () {
            this.deploy.autoRender && this.render();
        },
        ...
        set: function (arg, val) {
            ...
            if (Object.prototype.toString.call(this.deploy[arg]) === Object.prototype.toString.call(val)) {
                this.deploy[arg] = val
            } else {
                console.error("传值类型出错,应该是：", Object.prototype.toString.call(this.deploy[arg]));
            }
        },
        ...
        render: function () {
            var items = this.deploy.items, //取出配置item
                parEl = this.deploy.rendEl, //获取父元素
                common = this.deploy.common || null,
                inputMode = this.deploy.inputMode || null,
                relation = this.deploy.relation || null,
                isdisabled = this.deploy.disabled; //是否禁用

            var childEl = '';

            //清空重新绘制
            $(parEl).children().length && $(parEl).empty();

            //调用单元绘制
            var tpl = this.unitRender({
                elName: 'li',
                elData: items,
                classstr: 'input-selected-item',
                tpl: '<i class="icon-del-selected">×</i>'
            });

            //注入单元包裹层
            var ulEl = '<ul class="input-selected-list">' + tpl + '</ul>';
            var inpEl = isdisabled && '<input type="text" class="input-selected-text" disabled="disabled">' || '<input type="text" class="input-selected-text">';
            ...
            if (inputMode) {
                ...
                this.deploy.inputEls.val(valText);
            }
            ...
            //选中数据回调
            if (this.deploy.getSelectedData && typeof this.deploy.getSelectedData === 'function') {
                this.deploy.getSelectedData(this.deploy.items);
            }
        },
            ...
            if (parame.elData.length) {
                $(parame.elData).each(function (i, item) {
                    var elClass = item.elCls || '',
                        ...
                        childTpl = item.childTpl || '',
                        value = item.value || '',
                        defaultTpl = parame.tpl || '',
                        checkTpl = parame.checkTpl || '',
                        checked = item.checked ? 1 : 0,
                        disabledCls = item.disabled ? 'czy-disabled' : '';

                    unitEl += '<' + parame.elName + '  class="' + parame.classstr + " " + elClass + " " + disabledCls + '" id="' + itemId + '">' + checkTpl + itemText + childTpl + defaultTpl + '</' + parame.elName + '>';
                });
            }
            return unitEl
        },
        ...
        event: function () {
            ...
            //删除
            this.deploy.selectedEls.on('click', '.icon-del-selected', function (e) {
                var index = $(this).parent().index();
                if ($(this).parent().hasClass('czy-disabled') || _this.deploy.items[$(this).parent().index()].disabled) {
                    return
                }
                _this.delSelected(index);
            })

            //键盘输入事件
            _this.deploy.inputEls.on('keyup', function (e) {
                var val = $(this).val().replace(/(^\s*)|(\s*$)/g, "");
                _this.selectRender($(this), val);
            });

            //input获取焦点事件
            _this.deploy.inputEls.on('focus', function () {
                var val = $(this).val().replace(/(^\s*)|(\s*$)/g, "");
                if (_this.deploy.relation) {
                    _this.selectListRender($(this));
                }
            });

            //点击空白  消失
            $(document).on('click', function (e) {
                if (!$(e.target).parent().hasClass('input-search-select-list')) {
                    $('.input-search-select-list').empty();
                    $('.input-search-select-list').hide();
                }
            });
        },

        //关联渲染
        relationRender: function (){

        },

        //下拉选择渲染
        selectListRender: function ($currEl, result) {
            var _this = this;
            var elOffset = $currEl.offset();
            var result = result || this.deploy.datas;
            //渲染多选
            var checkEl = this.deploy.check ? '<span class="czy-checkbox"></span>' : '';
            var tpl = this.unitRender({
                elName: 'li',
                elData: result,
                classstr: 'search-select-item',
                checkTpl: checkEl
            });

            if (!$('.input-search-select-list').length) {
                //注入单元包裹层
                var ulEl = '<ul class="input-search-select-list">' + tpl + '</ul>';
                //注入父级元素
                $('body').append(ulEl);
            } else {
                $('.input-search-select-list').empty();
                $('.input-search-select-list').hide();
                tpl && $('.input-search-select-list').append(tpl) && $('.input-search-select-list').show();
            }

            //定位下拉框
            $('.input-search-select-list').css({
                top: elOffset.top + 35 + 'px',
                left: elOffset.left + 'px',
                maxHeight: this.deploy.height + 'px',
                width: this.deploy.inputEls.width() + 10 + 'px'
            })

            //获取所有选项元素
            this.deploy.selectItemEls = $('.input-search-select-list').children('li');

            //已选展示
            this.selectedFilter(result);

            //选择
            this.deploy.selectItemEls.on('click', function () {
                var $this = $(this);
                _this.selectedItem($this,result);
                _this.moveSelectWrap(); //移动选择框
            });
        },

        //已选展示
        selectedFilter:function(result){
            console.log(this.deploy.items);
            var _this = this;
            $(result).each(function (i, item1) {
                $(_this.deploy.items).each(function (inx, item2) {
                    if (item1.id === item2.id) {
                        $(_this.deploy.selectItemEls[i]).addClass('czy-selected');
                    }
                })
            });
        },

        selectedItem: function ($currItem,result) {
            var _this = this;
            var index = $currItem.index();
            var repeatIndex = _this.deWeight(result[index]);
            if (_this.deploy.check) {
                //多选
                if ($currItem.hasClass('czy-selected')) {
                    $currItem.removeClass('czy-selected');
                    _this.delSelected(repeatIndex);
                } else {
                    $currItem.addClass('czy-selected');
                    _this.addSelected(result[index]);
                }
                ...
                $('.input-search-select-list').empty();
                _this.deploy.inputEls.val();
            }
        },

        //搜索下拉选择渲染
        selectRender: function ($currEl, val) {
            var _this = this;
            var result = null;
            //是否调用外部接口回传数据
            if (_this.deploy.searchCb && typeof _this.deploy.searchCb === 'function') {
                result = val && this.deploy.searchCb(val) || [];
            } else {
                result = val && this.searchData(val) || [];
            }
            ...
            this.selectListRender($currEl, result);
        },

        //移动选择下拉的位置
        moveSelectWrap: function () {
            var inputOffset = this.deploy.inputEls.offset();
            $('.input-search-select-list').css({
                top: inputOffset.top + 35 + 'px',
        ...
        addSelected: function (item) {
            this.deploy.items.push(item);
            this.render();
        },

        //删除选中
        delSelected: function (index) {
            this.deploy.items.splice(index, 1);
            this.render();
        },

        //去重
        deWeight: function (currenDate) {
            var index = null;
            $(this.deploy.items).each(function (i, item) {
                if (item.id === currenDate.id) {
                    index = i;
                }
            });
            return index
        ...
        searchData: function (str) {
            var result = [];
            $(this.deploy.datas).each(function (i, item) {
        ...
        //绘制后初始化元素
        initData: function (parEl) {
            var rootEl = parEl || $(this.deploy.rendEl);
            this.deploy.selectedEls = rootEl.find('.input-selected-item');
            this.deploy.inputEls = rootEl.find('.input-selected-text');
        },
    }
...
/*function(){if(BUI.loaderScript.getAttribute("data-auto-use")=="false")return;BUI.use(["bui/common","bui/data","bui/list","bui/picker","bui/menu","bui/toolbar","bui/progressbar","bui/cookie","bui/form","bui/mask","bui/select","bui/tab","bui/calendar","bui/overlay","bui/editor","bui/grid","bui/tree","bui/tooltip",'bui/uploader'])}();*/

(function () {
    if (BUI.loaderScript.getAttribute('data-auto-use') == 'false') {
        return;
    }
    BUI.use(["bui/common", "bui/data", "bui/list", "bui/picker", "bui/menu", "bui/toolbar", "bui/progressbar", "bui/cookie", "bui/form", "bui/mask", "bui/select", "bui/tab", "bui/calendar", "bui/overlay", "bui/editor", "bui/grid", "bui/tree", "bui/tooltip", 'bui/uploader', 'czy/shrink', 'czy/slideThunk', 'czy/check', 'czy/menuSlide', 'czy/searchSelect']);
})();
