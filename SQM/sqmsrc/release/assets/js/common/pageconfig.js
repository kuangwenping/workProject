...
$('.input-search-select-list').mCustomScrollbar({
    axis:'y',
    theme:'dark',
});
...
seajs.use('bui/tree', function (Tree) {
    ...
    tree.on('itemclick', function (ev) {
        ...
        selectedRender(checkedNodes);
    });

    function selectedRender(checkedNodes){
        ...
        var $delBtns = $('.tree-selected-item #J_delSelceted');
        var sedItem = list.children();

        $delBtns.on('click',function(){
            var $curritem = $(this).parent();
            var node = tree.findNode($curritem.attr('data-id'));
            sedItem.splice($curritem.index(),1);
            selectedRender();
            console.log(sedItem);
            console.log(node);
