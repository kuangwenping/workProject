...
define('common/main',['bui/menu','bui/tab'],function(require) {
  ...
  //创建菜单和Tab，并绑定关联,是否收缩，是否有首页
    //参数：模块ID ，主菜单配置对象，次级菜单配置对象 ，是否收缩 , 首页
  function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){
    var _self =this,
      ...
      menuContainerEl = $(menuConfig.render),//获取次级菜单包裹
      slibEl = menuContainerEl.next('.' + CLS_LEFT_SLIB + '-con'),//滑动按钮
      navContainerEl = menuContainerEl.parents('.'+CLS_TAB_ITEM);//获取tab包裹层
  ...
  function getAutoHeight(){
    var height = BUI.viewportHeight(),//获取视窗
      subHeight = 60;
  ...
  BUI.augment(mainPage,{
    ...
    _createModule:function(id){
      ...
      module = new tabNav(id,{render:tabId,height:getAutoHeight() - 5},{render:treeId,items:item.menu,height:38},item.collapsed,item.homePage);
    ...
    _initNavItems : function(){
      ...
      var count =  navItems.length,
        ...
        //clientWidth = $("."+NAV_WRAP).width();  //获取导航栏宽度
        itemWidth = WIDTH_ITERM,//单个导航跨度（有待优化）
        totalWidth = itemWidth * count,//计算导航的总宽度
        showCount = 0;
    ...
    _initHideList : function(){
      ...
      var template = '<ul class="dl-hide-list ks-hidden"></ul>',
        hideListEl = $(template).appendTo('body');//放入body
      hideList = hideListEl;
    ...
    _initHideListEvent:function(){
      ...
      hideList.on('click',function(event){
        ...
        if(item){
          el = $(item);
          dataIndex = el.attr(ATTTR_INDEX);//获取点击项下标
          _self._setModuleSelected(dataIndex);
          _self._hideHideList();//隐藏
        }
      });
    },
    //初始化内容
    _initContents : function () {
      var _self = this,
        modulesConfig = _self.get('modulesConfig'),//获取配置项
        navContent = _self.get('navContent');//获取主导航对象
        mainContent = _self.get('mainContent');//获取主导航对象
      ...
      $.each(modulesConfig,function(index,module){
        var id = module.id,
          temp =['' +
          '<li class="dl-tab-item muen-wrap ks-hidden">' +
             '<div class="dl-second-nav ui-max-width">' +
                '<div class="dl-second-tree" id="J_',id,'Tree"></div>' +
              '</div>' +
          '</li>'].join('');
          var tempC = ['<div class="dl-inner-tab" id="J_',id,'Tab"></div>'].join('');
          new $(tempC).appendTo(mainContent);
    ...
    _initLocation :function (){
      ...
      if(defaultSetting){
        ...
        var currentModelIndex = _self.get('currentModelIndex'),
          moduleId = _self._getModuleId(currentModelIndex);//获取模块ID
    ...
    _getCurrentPageId : function(){
      var _self = this,
        moduleId = _self._getCurrentModuleId(),//获取当前模块配置ID
        module = _self._getModule(moduleId),//获取模块
        pageId ='';
      if(module){
        var item = module.menu.getSelected();//没走通
        if(item){
          pageId = item.get('id');
        }
      }
    ...
    _setModuleSelected : function(index,sender){
      var _self = this,
        navItems = _self.get('navItems'),//主导航
        navTabs = _self.get('navTabs'),//子导航
        currentModelIndex = _self.get('currentModelIndex'); //当前模块下标
      if(currentModelIndex !==index){
        var moduleId = _self._getModuleId(index),//获取模块配置ID
          module = null,
          lastShowItem = _self.get('lastShowItem'),
        ...
        setNavPosition(moduleId,curPageId);//更改链接地址
